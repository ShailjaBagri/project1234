...
def popular_cuisines():
	for row in cur.execute('SELECT nodes_tags.value, COUNT(*) as num             FROM nodes_tags                 JOIN (SELECT DISTINCT(id) FROM nodes_tags WHERE value="restaurant") i                 ON nodes_tags.id=i.id             WHERE nodes_tags.key="cuisine"             GROUP BY nodes_tags.value             ORDER BY num DESC'):
		return row

if __name__ == '__main__':
	
	con = sqlite3.connect("C:/Users/shailjabagri/Downloads/ahmedabad.db")
	cur = con.cursor()
